...
class Model {
    ...
    shininessMap = new Map(); // A map of material name to corresponding shininess value

    diffuseTextured = new Map();
    ambientTextured = new Map();
    diffuseTexturePaths = new Map();
    ambientTexturePaths = new Map();
    ...
    parseMtlFile(mtlFile) {
        ...
        for (let currLine = 0; currLine < mtlLines.length; currLine++) {
            ...
            if (line.startsWith("newmtl")) {        // Hit a new material
            ...
            else if (line.startsWith("Ns")) {       // Material specular definition
                let values = line.match(/[+-]?([0-9]+[.])?[0-9]+/g);
                this.shininessMap.set(currMaterial, [parseFloat(values[0])]);
            }
                ...
                this.diffuseTexturePaths.set(currMaterial, "https://ZP6400.github.io/Temp/model_files/" + line.substring(line.indexOf(' ') + 1));

                // Set textured flag
                this.diffuseTextured.set(currMaterial, true);

            } else if (line.startsWith("map_Ka")) {   // Texture file

                // Get the path of the texture file
                // Note that any model will have at most one texture file
                this.ambientTexturePaths.set(currMaterial, "https://ZP6400.github.io/Temp/model_files/" + line.substring(line.indexOf(' ') + 1));

                // Set textured flag
                this.ambientTextured.set(currMaterial, true);
